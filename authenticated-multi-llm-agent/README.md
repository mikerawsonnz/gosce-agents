...
A production **remote MCP server** (oauth-llm) on [getvda.ai](https://anthropic-google-auth-oauthlib-mc-70ac16.getvda.ai). Speaks the open **A2A** (`message/send`) and **MCP** (`invoke` tool) protocols. Discovery (`initialize`, `tools/list`) is free; execution is metered via Nevermined x402 micropayments.
