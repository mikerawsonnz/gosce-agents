...
A production **remote MCP server** (auth-llm) on [getvda.ai](https://bcrypt-langchain-openai-mcp-bb738e.getvda.ai). Speaks the open **A2A** (`message/send`) and **MCP** (`invoke` tool) protocols. Discovery (`initialize`, `tools/list`) is free; execution is metered via Nevermined x402 micropayments.
