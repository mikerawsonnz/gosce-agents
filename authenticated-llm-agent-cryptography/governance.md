# Governance — authenticated-llm-agent-cryptography
...
```
client → https://authenticated-llm-agent-cryptography.getvda.ai/{mcp | a2a}
       → capability dispatch (cryptography + google-auth-oauthlib + langchain + mcp + openai + opentelemetry-exporter-otlp + opentelemetry-sdk)
       → response
```
...
- **Step-by-step instructions:** `GET https://authenticated-llm-agent-cryptography.getvda.ai/verify` (free)
