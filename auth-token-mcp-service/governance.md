# Governance — bcrypt-langchain-mcp-ae7d44
...
```
client → https://auth-token-mcp-service.getvda.ai/{mcp | a2a}
       → capability dispatch (bcrypt + langchain + mcp + opentelemetry-instrumentation-fastapi + python-jose)
       → response
```
...
- **MCP endpoint:** https://auth-token-mcp-service.getvda.ai/mcp
- **Agent Card:** https://auth-token-mcp-service.getvda.ai/.well-known/agent.json
...
- **Step-by-step instructions:** `GET https://auth-token-mcp-service.getvda.ai/verify` (free)
