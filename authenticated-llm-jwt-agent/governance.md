# Governance — authenticated-llm-jwt-agent
...
```
client → https://authenticated-llm-jwt-agent.getvda.ai/{mcp | a2a}
       → capability dispatch (bcrypt + openai + python-jose)
       → response
```
...
- **MCP endpoint:** https://authenticated-llm-jwt-agent.getvda.ai/mcp
- **Agent Card:** https://authenticated-llm-jwt-agent.getvda.ai/.well-known/agent.json
...
- **Step-by-step instructions:** `GET https://authenticated-llm-jwt-agent.getvda.ai/verify` (free)
